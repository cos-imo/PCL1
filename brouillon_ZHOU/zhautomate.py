        ...
        while ind_cara_lu<len(code):
            if code[ind_cara_lu] not in self.alphabet:
                ...
                if resultat == []:
                    ...
                    # si il y a plusieurs mots qui commencent par token_courant, on prend le plus long
                    max_longueur = 0
                    meilleur_resultat = None
                    for r in resultat:
                        if len(r)>max_longueur and code.startswith(r, ind_cara_lu):
                            max_longueur = len(r)
                            meilleur_resultat = r
                    
                    if meilleur_resultat:
                        etat_courant = self.transitions[etat_courant][meilleur_resultat]
                        ind_cara_lu += max_longueur
                    else:
                        # si aucun mot ne commence par token_courant, alors erreur
                        return list_token, ligne, False
                
                if code[ind_cara_lu] == '\n':
                    # si saut de ligne alors on incrémente le numéro de ligne
                    ligne += 1
                
            # à la fin de la lecture si on est dans un état final 
            # on ajoute le token à la liste des tokens
            if etat_courant in self.etats_finaux:
                list_token.append((self.etats_finaux[etat_courant], token_courant))
                # on réinitialise le token courant et l'état courant
                token_courant = ''
                etat_courant = self.etat_initial
            
            #on renvoie la liste des tokens, le numéro de ligne et True
